...
# Quantization
def quantize_weights(adjacency_matrix):
    """ Convert from 32 bit to 8 bit int"""
    max_val = np.max(np.abs(adjacency_matrix))
    scale = 127 / max_val
    quantized_weights = np.round(adjacency_matrix * scale)
...
if __name__ == "__main__":
    # Load datasets
    cora_dataset = Planetoid(root='~/somewhere/Cora', name='Cora')
    pubmed_dataset = Planetoid(root='~/somewhere/PubMed', name='PubMed')
    citeseer_dataset = Planetoid(root='~/somewhere/Citeseer', name='Citeseer')

    datasets = {
        'cora.mtx': cora_dataset[0],
        'pubmed.mtx': pubmed_dataset[0],
        'citeseer.mtx': citeseer_dataset[0] 
    }
    ...
    for dataName, data in datasets.items():
      ...
      # Quantization technique on dense matrix
      quantize_adj_matrix = quantize_weights(pruned_dense_matrix)  
